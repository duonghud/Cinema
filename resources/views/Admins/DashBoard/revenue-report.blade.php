...
<style>
    ...
    .invoice-stat-row { display: flex; gap: 10px; margin-bottom: 18px; flex-wrap: wrap; }
    .invoice-stat {
    ...
    .inv-loading {
        display: flex; flex-direction: column;
        align-items: center; justify-content: center;
        padding: 44px 0; color: var(--muted); gap: 12px;
    }
    .inv-spinner {
        ...
        <div class="card-body py-3">
            <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3">
                ...
                <form method="GET" class="d-flex align-items-center gap-2">
                    <label for="filterValue" class="fw-semibold mb-0" style="color:var(--text);white-space:nowrap">
                        {{ $filterLabel }}
                    </label>
                    @if($filterKey === 'year')
                        <select name="{{ $filterKey }}" id="filterValue" class="form-select" style="border-color:var(--border)">
                    ...
                    @else
                        <input type="month" name="{{ $filterKey }}" id="filterValue"
                               value="{{ $filterValue }}" class="form-control" style="border-color:var(--border)">
                    @endif
    ...
    <div class="card report-card mb-4">
        <div class="card-body">
            ...
            <div class="chart-panel active" id="panel-bar">
                <small style="font-size:.78rem;color:var(--muted)" class="d-block mb-2">
                        ...
                        @forelse($rows as $row)
                            <tr data-row style="cursor:pointer"
                                ...
                                <td class="fw-semibold">
                                    {{ $row['label'] }}
                                    <small class="ms-1" style="color:var(--primary);font-size:.72rem"></small>
                                </td>
...
const periods       = @json($rows->pluck('period') ?? $rows->pluck('label'));
...
const C = {
    ticket:  '#6366f1',
    ticketA: 'rgba(99,102,241,0.15)',
    food:    '#06b6d4',
    foodA:   'rgba(6,182,212,0.12)',
    total:   '#10b981',
    totalA:  'rgba(16,185,129,0.10)',
};
...
const lineChart = new Chart(document.getElementById('lineChart'), {
    ...
    data: {
        labels,
        datasets: [
            { label:'Doanh thu vé',    data:ticketData, borderColor:C.ticket, backgroundColor:C.ticketA, tension:0.4, fill:true, pointBackgroundColor:C.ticket, pointRadius:5, pointHoverRadius:8 },
            { label:'Doanh thu đồ ăn', data:foodData,   borderColor:C.food,   backgroundColor:C.foodA,   tension:0.4, fill:true, pointBackgroundColor:C.food,   pointRadius:5, pointHoverRadius:8 },
            { label:'Tổng doanh thu',  data:totalData,  borderColor:C.total,  backgroundColor:C.totalA,  tension:0.4, fill:true, pointBackgroundColor:C.total,  pointRadius:5, pointHoverRadius:8, borderWidth:2.5 },
        ]
    },
    options: {
        responsive: true,
...
function openInvoiceModal(period, label) {
    ...
    const url = `/admins/reports/invoices-by-period?period=${encodeURIComponent(period)}`;
            ...
            <table class="table table-hover align-middle mb-0 modal-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="fw-semibold text-nowrap" style="color:#6366f1">INV-${inv.invoiceID}</td>
                        ...
                        <td>
                            <a href="/admins/invoices/${inv.invoiceID}"
                               class="btn btn-sm fw-semibold" target="_blank"
