...
<div class="container mt-4">
    ...
    @include('admins.partials.page-search', [
        'placeholder' => 'Tìm theo mã hoặc mô tả kiểm duyệt'
    ])
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    @forelse($ageRatings as $item)
                        <tr>
                            <td class="text-muted">{{ $item->ageRatingID }}</td>
                            <td>
                                <span class="badge bg-warning text-dark px-3 py-2">{{ $item->code }}</span>
                            </td>
                            <td>
                                <span class="text-secondary">{{ $item->description }}</span>
                            </td>
                            <td class="text-end">
                                <a href="{{ route('ageRating.edit', $item) }}" class="btn btn-sm btn-outline-dark me-2">
                                    Sửa
                                </a>

                                <form action="{{ route('ageRating.destroy', $item) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
